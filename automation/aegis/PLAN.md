...
30B-A3B (~18 GB) + 16k KV (~1–2 GB) + embed model (~0.5 GB) + Python/LangGraph/Chainlit/Chroma (~1–2 GB) ≈ **21–23 GB**. Fits — **but only if exactly one large model is resident.** Never load a 2nd big model for a "verifier" pass; reuse the same model with a critic prompt, or a small model. Tune Ollama `keep_alive` / unload the reasoner when idle to protect the 24 GB ceiling during design-tool/browser multitasking. Zero-swap is achievable; make it a monitored metric, not an assumption.
